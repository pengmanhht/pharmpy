...
def score_test_add_covariate_effect(
    model: Model,
    parameter: str,
    covariate: str,
    effect: EffectType,
    ...
    covariate_effect = _create_template_score_test(effect, model, covariate)
    pset, thetas = _create_thetas_score_test(
        model, parameter, effect, covariate, covariate_effect.template
    )
    covariate_effect.apply(parameter, covariate, thetas, statistics)
    # NOTE: We hoist the statistic statements to avoid referencing variables
    # before declaring them. We also avoid duplicate statements.
...
def _create_thetas_score_test(
    model, parameter, effect, covariate, template, _ctre=re.compile(r"theta\d*")
):
    """Creates theta parameters and adds to parameter set of model.
    ...
    if no_of_thetas == 1:
        ...
        theta_name = f"POP_{parameter}{covariate}"
        pset = Parameters.create(
            list(pset) + [Parameter(theta_name, 1.0, inits["lower"], inits["upper"])]
        )
        theta_names["theta"] = theta_name
    else:
        for i, new_theta in enumerate(sorted(new_thetas, key=natural_order), 1):
...
def _create_template_score_test(effect, model, covariate):
    """Creates Covariate class objects with effect template."""
    if effect == "lin":
        return ScoreTestCovariateEffect.linear()
    elif effect == "cat":
        counts = _count_categorical(model, covariate)
        return ScoreTestCovariateEffect.categorical(counts)
    elif effect == "cat2":
        counts = _count_categorical(model, covariate)
        return ScoreTestCovariateEffect.categorical(counts, alternative=True)
    elif effect == "piece_lin":
        return ScoreTestCovariateEffect.piecewise_linear()
    elif effect == "exp":
        return ScoreTestCovariateEffect.exponential()
    elif effect == "pow":
        return ScoreTestCovariateEffect.power()
    else:
        symbol = Expr.symbol("symbol")
        expression = parse_expr(effect)
        return ScoreTestCovariateEffect(Assignment.create(symbol, expression))
...
class ScoreTestCovariateEffect(CovariateEffect):
    def __init__(self, template):
    ...
    def linear(cls):
        """Linear continuous template (for continuous covariates)."""
        symbol = Expr.symbol("symbol")
        expression = 1 + (Expr.symbol("theta") - 1) * (Expr.symbol("cov") - Expr.symbol("median"))
        template = Assignment.create(symbol, expression)
        ...
        for i, cat in enumerate(categories, 1):
            if cat != most_common:
                if np.isnan(cat):
                    ...
                    if len(categories) == 2:
                        if alternative:
                            values += [Expr.symbol("theta") - 1]
                        else:
                            values += [1 + (Expr.symbol("theta") - 1)]
                    else:
                        if alternative:
                            values += [Expr.symbol(f"theta{i}") - 1]
                        else:
                            values += [1 + (Expr.symbol(f"theta{i}") - 1)]
    ...
    def piecewise_linear(cls):
        ...
        symbol = Expr.symbol("symbol")
        values = [
            1 + (Expr.symbol("theta1") - 1) * (Expr.symbol("cov") - Expr.symbol("median")),
            1 + (Expr.symbol("theta2") - 1) * (Expr.symbol("cov") - Expr.symbol("median")),
        ]
        conditions = [
            BooleanExpr.le(Expr.symbol("cov"), Expr.symbol("median")),
            BooleanExpr.gt(Expr.symbol("cov"), Expr.symbol("median")),
    ...
    def exponential(cls):
        ...
        symbol = Expr.symbol("symbol")
        expression = Expr.exp(
            (Expr.symbol("theta") - 1) * (Expr.symbol("cov") - Expr.symbol("median"))
        )
        template = Assignment.create(symbol, expression)
    ...
    def power(cls):
        """Power template (for continuous covariates)."""
        symbol = Expr.symbol("symbol")
        expression = (Expr.symbol("cov") / Expr.symbol("median")) ** (Expr.symbol("theta") - 1)
        template = Assignment.create(symbol, expression)
